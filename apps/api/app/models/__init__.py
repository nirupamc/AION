...
class SavedFlow(TimestampMixin, Base):
    """A persisted Smart Flow sequence generated by the M9 optimizer."""

    __tablename__ = "saved_flows"
    __table_args__ = (
        Index("ix_saved_flows_created", "created_at"),
    )
    ...
    start_track_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("tracks.id", ondelete="SET NULL"), nullable=True
    )
    target_track_count: Mapped[int] = mapped_column(Integer, nullable=False)
    energy_shape: Mapped[str] = mapped_column(String(32), nullable=False)
    constraints_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # Scoring summary
    overall_sequence_score: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    average_transition_score: Mapped[Optional[float]] = mapped_column(nullable=True)
    minimum_transition_score: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)

    # Provenance
    optimizer_version: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    transition_model_version: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="ok")

    tracks: Mapped[list["SavedFlowTrack"]] = relationship(
        back_populates="flow",
        cascade="all, delete-orphan",
        order_by="SavedFlowTrack.position",
    )
    exports: Mapped[list["FlowExport"]] = relationship(
        back_populates="flow",
        cascade="all, delete-orphan",
    )


class SavedFlowTrack(TimestampMixin, Base):
    """A single track in a saved flow, preserving exact order and transition data."""

    __tablename__ = "saved_flow_tracks"
    __table_args__ = (
        UniqueConstraint("flow_id", "position", name="uq_saved_flow_track_position"),
        Index("ix_saved_flow_tracks_flow", "flow_id"),
        Index("ix_saved_flow_tracks_track", "track_id"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    flow_id: Mapped[int] = mapped_column(
        ForeignKey("saved_flows.id", ondelete="CASCADE"), nullable=False
    )
    track_id: Mapped[int] = mapped_column(
        ForeignKey("tracks.id", ondelete="CASCADE"), nullable=False
    )
    ...
    # Track metadata snapshot (denormalized for fast export)
    ...
    camelot: Mapped[Optional[str]] = mapped_column(String(8), nullable=True)
    energy: Mapped[Optional[float]] = mapped_column(nullable=True)
    dominant_mood: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    dominant_vibe: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)

    # Transition from previous track
    transition_score: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    transition_components_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    transition_reasons_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    transition_warnings_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    flow: Mapped[SavedFlow] = relationship(back_populates="tracks")
    track: Mapped[Track] = relationship()


class FlowExport(TimestampMixin, Base):
    """Audit record of a flow export to an external provider."""

    __tablename__ = "flow_exports"
    __table_args__ = (
        Index("ix_flow_exports_flow", "flow_id"),
    )
    ...
    provider: Mapped[str] = mapped_column(String(64), nullable=False)
    external_playlist_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    external_playlist_url: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    external_playlist_name: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    exported_track_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    skipped_track_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    skipped_tracks_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="success")
    error_summary: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    flow: Mapped[SavedFlow] = relationship(back_populates="exports")
